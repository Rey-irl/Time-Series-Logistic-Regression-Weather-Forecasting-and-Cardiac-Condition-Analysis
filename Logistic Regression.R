...
condition<-ggplot(data=cardiac_df, aes(y=cardiac_condition)) + geom_bar(fill="steelblue", color="black") +
              ggtitle("Bar chart of Cardian Condition Values")
ggplotly(condition)
...
#=================================
summary(df)
#Create a dataframe with only numeric variable
df2<-df %>% select('age','weight','fitness_score')
head(df2)
autoplot(prcomp(df2, scale. = T, center = T), data = df, colour = "cardiac_condition")

#Contribution of explained variance of each component
screeplot(prcomp(df2, scale. = T, center = T), type = "lines")

#Apply PCA 
pca <- PCA(df2, scale.unit = T, ncp = 3)
pca
summary (pca)

#plot of components number
fviz_screeplot(pca, addlabels = TRUE)

fviz_pca_ind(pca, habillage = df$cardiac_condition, label = "none", addEllipses = T)
